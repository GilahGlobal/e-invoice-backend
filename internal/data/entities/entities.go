...
type Business struct {
	...
	CreatedByAggregator bool                   `gorm:"column:created_by_aggregator;type:bool;default:false" json:"created_by_aggregator"`
	BmpUploadSelected   bool                   `gorm:"column:bmp_upload_selected;type:bool;default:false" json:"bmp_upload_selected"`
...
func (pc *PlatformConfigs) Scan(value interface{}) error {
	...
	case map[string]interface{}:
		...
		if err != nil {
			return fmt.Errorf("failed to re-marshal native map: %v", err)
		}
		bytes = b
	default:
		// Attempt to JSON marshal whatever it is, as a fallback
		b, err := json.Marshal(v)
		if err != nil {
			return fmt.Errorf("failed to unmarshal PlatformConfigs: unsupported type %T (value: %v)", value, value)
		}
		bytes = b
	}
